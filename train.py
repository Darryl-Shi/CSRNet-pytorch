...
class CSRNetLightning(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        wandb_logger.log_image(str(self.current_epoch)+'/Image', t(denormalize(image[0].cpu())))
        wandb_logger.log_image(str(self.current_epoch)+'/Estimate density count:'+ str('%.2f'%(et_densitymap[0].cpu().sum())), t(et_densitymap[0]/torch.max(et_densitymap[0])))
        wandb_logger.log_image(str(self.current_epoch)+'/Ground Truth count:'+ str('%.2f'%(gt_densitymap[0].cpu().sum())), t(gt_densitymap[0]/torch.max(gt_densitymap[0])))
        return mae
